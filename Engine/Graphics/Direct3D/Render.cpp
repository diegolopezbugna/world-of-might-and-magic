...
void Render::RenderTerrainD3D() {  // New function
    ...
    for (unsigned int z = Start_Z; z < End_Z; ++z) {
        for (unsigned int x = Start_X; x < End_X; ++x) {
            ...
            uint norm_idx = pTerrainNormalIndices[2 * ((x) * 128 + z+1) /*+ 1*/];
...
bool Render::DrawLightmap(Lightmap *pLightmap, Vec3_float_ *pColorMult,
    ...
    for (uint i = 0; i < pLightmap->NumVertices; ++i) {
        ...
        if (fabs(z_bias) < 1e-5) {
            //v18 = 1.0 -
            //      1.0 / ((1.0f / (8192/*pIndoorCameraD3D->GetFarClip()/4*/)) *
            //             pLightmap->pVertices[i].vWorldViewPosition.x * 1000.0);
            /*v18 = 1.0 -
                1.0 / (pLightmap->pVertices[i].vWorldViewPosition.x * 0.061758894);*/
            //v18 = 0.99;
            /*v18 = 1.0 - 1.0 / ((1.0f / pIndoorCameraD3D->GetShadingDistMist()) * pLightmap->pVertices[i].vWorldViewPosition.x * 1000.0);*/ //orig
            ...
            //    z_bias;
            /*v18 = 1.0 -
                1.0 / (pLightmap->pVertices[i].vWorldViewPosition.x * 0.061758894) - z_bias;*/
            //v18 = 0.99 - z_bias;
        ...
        if (pVerticesD3D[i].diffuse < 0) __debugbreak();
        ...
        if (abs(pVerticesD3D[i].texcoord.y) > 1 ) {
            __debugbreak();
    }
    }
        ...
        if (!lightmap_builder->StationaryLightsCount ||
           ...
           // return;
            ...
            for (uint i = 0; i < uNumVertices; ++i) {
                d3d_vertex_buffer[i].pos.x = array_507D30[i].vWorldViewProjX;
                d3d_vertex_buffer[i].pos.y = array_507D30[i].vWorldViewProjY;
                d3d_vertex_buffer[i].pos.z = //0.99;
                    1.0 -
                    1.0 / (array_507D30[i].vWorldViewPosition.x * 0.061758894);
            ...
            //test
           // ErrD3D(pRenderD3D->pDevice->SetRenderState(D3DRENDERSTATE_ZFUNC, 1));
...
void Render::SetBillboardBlendOptions(RenderBillboardD3D::OpacityType a1) {
    switch (a1) {
        ...
        default: {
            ...
            assert(false); // fireball and impolision can crash here - maybe clipping relatee
            break;
        }
    }
}
